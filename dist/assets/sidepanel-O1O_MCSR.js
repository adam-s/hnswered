import { R as RETENTION } from './constants-CC4aYNRT.js';
...
const api = {
  listReplies: () => send({ kind: "list-replies" }),
  markRead: (id) => send({ kind: "mark-read", id }),
...
var root_1$3 = from_html(`<a class="quoted" target="_blank" rel="noopener"><span class="quote-author"> </span> <span class="quote-text"> </span></a>`);
var root_2$3 = from_html(`<a class="quoted story" target="_blank" rel="noopener"><span class="quote-text"> </span></a>`);
var root_3$2 = from_html(`<span class="dot">·</span> <button type="button">mark read</button>`, 1);
var root$3 = from_html(`<div><!> <div class="meta"><strong> </strong> <span class="dot">·</span> <a> </a></div> <div class="text"></div> <div class="actions"><a>open on hn</a> <!></div></div>`);

function ReplyRow($$anchor, $$props) {
	push($$props, true);

	const safeHtml = user_derived(() => sanitizeCommentHtml($$props.reply.text || '<em>(no text)</em>'));
	const hnLink = user_derived(() => `https://news.ycombinator.com/item?id=${$$props.reply.id}`);
	const parentLink = user_derived(() => `https://news.ycombinator.com/item?id=${$$props.reply.parentItemId}`);

	async function open() {
		window.open(get(hnLink), '_blank', 'noopener');

		if (!$$props.reply.read) await api.markRead($$props.reply.id);
	}
	...
	async function openParent(e) {
		e.preventDefault();
		window.open(get(parentLink), '_blank', 'noopener');
...

	{
		var consequent = ($$anchor) => {
			...
			template_effect(() => {
				set_attribute(a, 'href', get(parentLink));
				set_text(text, $$props.reply.parentAuthor ?? 'parent');
				set_text(text_1, $$props.reply.parentExcerpt);
			});

			delegated('click', a, openParent);
			append($$anchor, a);
		};
		...
		var consequent_1 = ($$anchor) => {
			...
			template_effect(() => {
				set_attribute(a_1, 'href', get(parentLink));
				set_text(text_2, $$props.reply.parentItemTitle);
			});

			delegated('click', a_1, openParent);
			append($$anchor, a_1);
		};

		if_block(node, ($$render) => {
			if ($$props.reply.parentExcerpt) $$render(consequent); else if ($$props.reply.parentItemTitle) $$render(consequent_1, 1);
		});
	}
	...
	var a_2 = sibling(strong, 4);
	var text_4 = child(a_2);

	var div_2 = sibling(div_1, 2);

	html(div_2, () => get(safeHtml), true);

	var div_3 = sibling(div_2, 2);
	var a_3 = child(div_3);
	var node_1 = sibling(a_3, 2);

	{
		var consequent_2 = ($$anchor) => {
			...
			delegated('click', button, markRead);
			append($$anchor, fragment);
		};

		if_block(node_1, ($$render) => {
			if (!$$props.reply.read) $$render(consequent_2);
		});
	}

	template_effect(
		($0) => {
			classes = set_class(div, 1, 'reply', null, classes, { read: $$props.reply.read });
			set_text(text_3, $$props.reply.author);
			set_attribute(a_2, 'href', get(hnLink));
			set_text(text_4, $0);
			set_attribute(a_3, 'href', get(hnLink));
		},
		[() => timeAgo($$props.reply.time)]
	);
...
var root_1$2 = from_html(`<div class="modal-title"> </div>`);
var root_2$2 = from_html(`<span class="sep">·</span> <button type="button" class="link gray"> </button>`, 1);
var root$2 = from_html(`<div class="modal-backdrop" role="presentation"><div class="modal" role="dialog" aria-modal="true"><!> <div class="modal-msg"> </div> <div class="modal-actions"><button type="button" class="link gray"> </button> <!> <span class="sep">·</span> <button type="button"> </button></div></div></div>`);

function Confirm($$anchor, $$props) {
	push($$props, true);

	let confirmLabel = prop($$props, 'confirmLabel', 3, 'confirm'),
		cancelLabel = prop($$props, 'cancelLabel', 3, 'cancel'),
		destructive = prop($$props, 'destructive', 3, false);

	let confirmBtn = state(void 0);
	let cancelBtn = state(void 0);
	...
	onMount(async () => {
		...
		if (destructive()) get(cancelBtn)?.focus(); else get(confirmBtn)?.focus();
	...
	bind_this(button, ($$value) => set(cancelBtn, $$value), () => get(cancelBtn));
...

	{
		var consequent_1 = ($$anchor) => {
			...
			var button_1 = sibling(first_child(fragment), 2);
			var text_3 = child(button_1);
			template_effect(() => set_text(text_3, $$props.secondaryLabel));

			delegated('click', button_1, function (...$$args) {
				$$props.onSecondary?.apply(this, $$args);
			});
		...
		if_block(node_1, ($$render) => {
			if ($$props.secondaryLabel && $$props.onSecondary) $$render(consequent_1);
		});
	}

	var button_2 = sibling(node_1, 4);
	let classes;
	...
	template_effect(() => {
		...
		set_text(text_2, cancelLabel());
		classes = set_class(button_2, 1, 'link', null, classes, { destructive: destructive() });
...
function Settings($$anchor, $$props) {
	...
	const initialHnUser = $$props.config.hnUser;
	...
	// svelte-ignore state_referenced_locally
	let tickMinutes = state(proxy($$props.config.tickMinutes));
	...
	function requestDone() {
		if (get(dirty)) set(modal, 'unsaved-changes'); else $$props.onDone();
	}
	...
	function onClearRead() {
		if (!get(stats) || get(stats).replyCount === get(stats).unreadCount) return;

		set(modal, 'clear-read');
	}

	async function doClearRead() {
		set(modal, 'none');
		set(clearingRead, true);

		try {
			await api.clearRead();
			await loadStats();
		} finally {
			set(clearingRead, false);
		}
	}

	function onClearAll() {
		if (!get(stats) || get(stats).replyCount === 0) return;
	...
	each(select, 21, () => intervals, index, ($$anchor, n) => {
		var option = root_1$1();
		...
		template_effect(() => {
			set_text(text, `${get(n) ?? ''} minute${get(n) === 1 ? '' : 's'}`);

			if (option_value !== (option_value = get(n))) {
				option.value = (option.__value = get(n)) ?? '';
			}
		});
	...
	each(select_1, 21, () => retentions, index, ($$anchor, n) => {
		...
		template_effect(() => {
			set_text(text_1, `${get(n) ?? ''} days`);

			if (option_1_value !== (option_1_value = get(n))) {
				option_1.value = (option_1.__value = get(n)) ?? '';
			}
		});
...

	{
		var consequent_2 = ($$anchor) => {
...

			{
				var consequent = ($$anchor) => {
					...
					delegated('click', button_1, onClearRead);
					append($$anchor, button_1);
				};

				if_block(node_1, ($$render) => {
					if (get(stats).replyCount > get(stats).unreadCount) $$render(consequent);
				});
			}
...

			{
				...
				if_block(node_2, ($$render) => {
					if (get(stats).replyCount > 0) $$render(consequent_1);
				});
			}
		...
		if_block(node, ($$render) => {
			if (get(stats)) $$render(consequent_2); else $$render(alternate, -1);
		});
	}
...

	{
		var consequent_3 = ($$anchor) => {
			{
				let $0 = user_derived(() => `Deletes ${get(stats).replyCount - get(stats).unreadCount} read ${get(stats).replyCount - get(stats).unreadCount === 1 ? 'reply' : 'replies'} from local storage. Unread are kept.`);
		...
		var consequent_4 = ($$anchor) => {
			{
				let $0 = user_derived(() => `Deletes all ${get(stats).replyCount} ${get(stats).replyCount === 1 ? 'reply' : 'replies'} from local storage, including ${get(stats).unreadCount} unread. Monitored items stay.`);
		...
		if_block(node_3, ($$render) => {
			if (get(modal) === 'clear-read' && get(stats)) $$render(consequent_3); else if (get(modal) === 'clear-all' && get(stats)) $$render(consequent_4, 1); else if (get(modal) === 'unsaved-changes') $$render(consequent_5, 2);
		});
	}
...
var root_2 = from_html(`<span class="dim">watching</span> <strong> </strong>`, 1);
var root_3 = from_html(`<span class="dim">no user configured</span>`);
var root_4 = from_html(`<button type="button">refresh</button> <span class="dot">·</span> <button type="button">mark all</button> <span class="dot">·</span>`, 1);
var root_1 = from_html(`<span class="who"><!></span> <span class="verbs"><!> <button type="button">settings</button></span>`, 1);
var root_5 = from_html(`<span class="who"><span class="dim">settings</span></span> <span class="verbs"><button type="button">done</button></span>`, 1);
var root_6 = from_html(`<div class="subbar"><button type="button">all <span class="count"> </span></button> <span class="sep">|</span> <button type="button">unread <span class="count"> </span></button> <span class="sep">|</span> <button type="button">read <span class="count"> </span></button></div>`);
var root_9 = from_html(`<div class="status">loading…</div>`);
var root_10 = from_html(`<div class="empty">No HN username configured. <a href="#">Configure →</a></div>`);
var root_11 = from_html(`<div class="empty"><!></div>`);
var root_17 = from_html(`<div class="more"><a href="#">More</a></div>`);
var root_15 = from_html(`<!> <!>`, 1);
var root_8 = from_html(`<div class="body"><!></div>`);
var root = from_html(`<div class="topbar"><!></div> <!> <!>`, 1);

function App($$anchor, $$props) {
	push($$props, true);

	let view = state('list');
	let replies = state(proxy([]));
	let config = state(proxy({ hnUser: '', tickMinutes: 5, retentionDays: 30 }));
	...
	const unreadCount = user_derived(() => get(replies).filter((r) => !r.read).length);
	const readCount = user_derived(() => get(replies).length - get(unreadCount));

	const filtered = user_derived(() => get(filter) === 'all'
		? get(replies)
		: get(filter) === 'unread'
			? get(replies).filter((r) => !r.read)
			: get(replies).filter((r) => r.read));

	const visible = user_derived(() => get(filtered).slice(0, get(renderLimit)));
	const hiddenCount = user_derived(() => Math.max(0, get(filtered).length - get(visible).length));
	...
	function setFilter(f) {
		if (get(filter) !== f) {
			...
			set(
				renderLimit,
				RETENTION.PAGE_SIZE, // reset paging on filter change
				true
			);
		}
	}
	...
	function loadMore() {
		set(renderLimit, get(renderLimit) + RETENTION.PAGE_SIZE);
	}

	// Route the topbar "done" click through Settings so it can prompt on unsaved changes.
	let settingsRef = state(void 0);

	function onDoneClick() {
		if (get(settingsRef)) get(settingsRef).requestDone(); else showList();
	}
...

	{
		var consequent_2 = ($$anchor) => {
...

			{
				var consequent = ($$anchor) => {
					...
					var strong = sibling(first_child(fragment_2), 2);
					var text = child(strong);
					template_effect(() => set_text(text, get(config).hnUser));
				...
				if_block(node_1, ($$render) => {
					if (get(config).hnUser) $$render(consequent); else $$render(alternate, -1);
				});
			}
...

			{
				var consequent_1 = ($$anchor) => {
					...
					template_effect(() => button_1.disabled = get(unreadCount) === 0);
					delegated('click', button, onRefreshClick);
					delegated('click', button_1, markAll);
					append($$anchor, fragment_3);
				};

				if_block(node_2, ($$render) => {
					if (get(config).hnUser) $$render(consequent_1);
				});
			}

			var button_2 = sibling(node_2, 2);
			delegated('click', button_2, showSettings);
			append($$anchor, fragment_1);
		};
		...
		var alternate_1 = ($$anchor) => {
			...
			var button_3 = child(span_3);
			delegated('click', button_3, onDoneClick);
			append($$anchor, fragment_4);
		};

		if_block(node, ($$render) => {
			if (get(view) === 'list') $$render(consequent_2); else $$render(alternate_1, -1);
		});
	}
...

	{
		var consequent_3 = ($$anchor) => {
			...
			let classes;
			var span_4 = sibling(child(button_4));
			var text_1 = child(span_4);

			var button_5 = sibling(button_4, 4);
			let classes_1;
			var span_5 = sibling(child(button_5));
			var text_2 = child(span_5);

			var button_6 = sibling(button_5, 4);
			let classes_2;
			var span_6 = sibling(child(button_6));
			var text_3 = child(span_6);

			template_effect(() => {
				classes = set_class(button_4, 1, '', null, classes, { active: get(filter) === 'all' });
				set_text(text_1, get(replies).length);
				classes_1 = set_class(button_5, 1, '', null, classes_1, { active: get(filter) === 'unread' });
				set_text(text_2, get(unreadCount));
				classes_2 = set_class(button_6, 1, '', null, classes_2, { active: get(filter) === 'read' });
				set_text(text_3, get(readCount));
			});

			delegated('click', button_4, () => setFilter('all'));
			delegated('click', button_5, () => setFilter('unread'));
			delegated('click', button_6, () => setFilter('read'));
			append($$anchor, div_1);
		};

		if_block(node_3, ($$render) => {
			if (get(view) === 'list' && get(config).hnUser) $$render(consequent_3);
		});
	}
...

	{
		var consequent_4 = ($$anchor) => {
			bind_this(
				...
				($$value) => set(settingsRef, $$value, true),
				() => get(settingsRef)
			);
		};
		...
		var alternate_4 = ($$anchor) => {
...

			{
				...
				var consequent_9 = ($$anchor) => {
...

					{
						var consequent_7 = ($$anchor) => {
							var text_4 = text('No new replies.');

							append($$anchor, text_4);
						};

						var consequent_8 = ($$anchor) => {
							var text_5 = text('Nothing read yet.');

							append($$anchor, text_5);
						};

						var alternate_2 = ($$anchor) => {
							var text_6 = text('No replies.');

							append($$anchor, text_6);
						};

						if_block(node_6, ($$render) => {
							if (get(filter) === 'unread') $$render(consequent_7); else if (get(filter) === 'read') $$render(consequent_8, 1); else $$render(alternate_2, -1);
						});
					}
					append($$anchor, div_5);
				};
				...
				var alternate_3 = ($$anchor) => {
					...
					each(node_7, 17, () => get(visible), (r) => r.id, ($$anchor, r) => {
						ReplyRow($$anchor, {
							get reply() {
								return get(r);
							}
				...
				if_block(node_5, ($$render) => {
					if (get(loading)) $$render(consequent_5); else if (!get(config).hnUser) $$render(consequent_6, 1); else if (get(filtered).length === 0) $$render(consequent_9, 2); else $$render(alternate_3, -1);
				});
			}
			append($$anchor, div_2);
		};
		...
		if_block(node_4, ($$render) => {
			if (get(view) === 'settings') $$render(consequent_4); else $$render(alternate_4, -1);
		});
	}
